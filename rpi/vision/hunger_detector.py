...
class HungerDetector:
                ...
                if mask.any():
                    ...
                    for idx, box in enumerate(xyxy):
                        ...
                        det_item = {
                            ...
                            "center": [round(cx, 2), round((by1 + by2) / 2.0, 2)],
                            "confidence": conf,
                            "in_feeding_roi": in_roi,
                        }
                        detections.append(det_item)
                        if in_roi:
                            feeding_roi_detections.append(det_item)
                        else:
                            ignored_detections.append(det_item)

                    
                    hungry_count = min(len(feeding_roi_detections), 4)
                    hungry_ids = [i + 1 for i in range(hungry_count)]
                    mean_conf = (
                        round(float(np.mean([d["confidence"] for d in feeding_roi_detections[:hungry_count]])), 3)
                        if hungry_count > 0 else 0.0
                    )
                    ...
                    return {
                        ...
                        "feeding_roi_detections": feeding_roi_detections[:4],
                        "ignored_detections": ignored_detections,
                        "feeding_roi": [int(mid_x), 0, int(orig_w), int(orig_h)],
                        "source": "yolov8_onnx",
                    }
                else:
                    mid_x = orig_w / 2.0
    ...
    def _contour_fallback(self, frame) -> Dict[str, Any]:
        """Classical CV contour analysis fallback on RHS feeding ROI when ONNX model is unavailable."""
        try:
            import cv2
