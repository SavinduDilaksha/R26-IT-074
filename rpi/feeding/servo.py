...
class FeederServo:
    ...
    def is_in_cooldown(self) -> Tuple[bool, float]:
        ...
        if elapsed < 0:
            LOG.warning("Clock drift detected in feeder cooldown (elapsed=%.1fs < 0). Clearing stale cooldown.", elapsed)
    ...
    def reset_cooldown(self) -> None:
        """Clear the post-feed cooldown timer completely."""
        self.last_dispense_time = None
        feed_file = DATA_DIR / "latest_feed.json"
        try:
            if feed_file.exists():
                data = load_json(feed_file, {})
                if data:
                    data["dispensed"] = False
                    data["last_dispense_time"] = None
                    from storage.json_store import save_json
                    save_json(feed_file, data)
        except Exception:
            pass
        LOG.info("Feeder post-dispense cooldown cleared.")
    ...
    def _actuate_hardware_pwm(self, angle: int = 180, rounds: int = 1) -> bool:
        ...
        try:
            ...
            duty_home = 2.5  
            duty_target = 2.5 + (angle / 180.0) * 10.0  
