...
def check_directories_and_artifacts():
    ...
    if FISH_MODEL_ONNX_PATH.exists():
        reporter.add_result(cat, "YOLOv8 Fish Detector (ONNX)", "PASS", f"Found ({FISH_MODEL_ONNX_PATH.stat().st_size} bytes)")
    else:
        reporter.add_result(cat, "YOLOv8 Fish Detector (ONNX)", "WARN", f"Missing at {FISH_MODEL_ONNX_PATH.relative_to(BASE_DIR)}")
...
def check_hardware_interfaces():
    ...
    if os.name == "nt" or (Path(ard_configured).exists() if ard_configured else False):
        reporter.add_result(cat, "Arduino Uno USB Serial Port", "PASS", f"Port interface available ({ard_configured})")
    else:
        reporter.add_result(cat, "Arduino Uno USB Serial Port", "WARN", f"Configured serial port ({ard_configured}) not attached")

    # Ion Concentration Serial Port
    ion_configured = SENSOR_CONFIG.get("ionconcentration_serial_port", "/dev/ttyUSB0")
    if os.name == "nt" or (Path(ion_configured).exists() if ion_configured else False):
        reporter.add_result(cat, "Ion Concentration Modbus Serial Port", "PASS", f"Port interface available ({ion_configured})")
    else:
        reporter.add_result(cat, "Ion Concentration Modbus Serial Port", "WARN", f"Configured serial port ({ion_configured}) not attached")
...
def check_core_modules():
    ...
    modules_to_test = [
        ...
        ("firebase.upload_feeding", "Firebase Feeder & Hunger Uploader"),
    ]
...
def check_functional_contracts():
    ...
    try:
        ...
        servo = FeederServo()
        rounds = servo.calculate_rounds(hungry_count=2)
        angle = servo.calculate_angle(hungry_count=2)
        reporter.add_result(cat, "Feeder Servo Logic", "PASS", f"Hungry count 2 -> Rounds {rounds} (Full CW/CCW {angle}°)")
    except Exception as exc:
    ...
    try:
        ...
        reporter.add_result(
            ...
            f"ROI mode={res_roi.get('disease_class')} | Full mode={res_full.get('disease_class')}"
        )
    except Exception as exc:
